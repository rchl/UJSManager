...
function handleRequest( event )
{
  ...
  {
    var tpldata = {
      msg       : $('<p>If you are the owner of this service, go to <a href="' + SERVICE_PATH_ADMIN + '" onclick="location.replace(this.href);return false;">Admin section</a>.</p>'),
      footer    : $('<p id="footer">Generated by <a href="http://unite.opera.com/application/401/">UJS Manager Opera Unite application</a>.</p>'),
      scripts   : ScriptsDirectory.getSharedUserScripts(),
...
var ScriptsDirectory = new function()
{
  ...
  /**
    * Called when remote user accesses public page
    * @returns object with scripts array containing scripts shared by owner
    */
  this.getSharedUserScripts = function()
  {
    var File, scripts = [], new_shared_scripts = [];

    for (var i=0; i<DATA.scripts.length; i++)
    {
      if (!DATA.scripts[i].isdirectory && shared_scripts.include(DATA.scripts[i].printpath))
      {
        scripts.push(DATA.scripts[i]);
        new_shared_scripts.push(DATA.scripts[i].printpath);
      }
    }

    shared_scripts = new_shared_scripts;
    return scripts;
  }
